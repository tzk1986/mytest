...
class ProjectPage(Page):
    ...
    def AddProject(self, name, num, version):
        ...
        self.sel_loc.select_by_index(num)
        self.vers_loc.send_keys(version)
        self.save_loc.click()
    ...
    def SearchProject(self, name=None):
        ...
        self.search_loc.send_keys(name)
        logger.info("点击搜索")
        self.search_btn_loc.click()

    def ModifyProject(self, version):
        logger.info("删除文本")
        self.mod_vers_loc.clear()
        logger.info("输入文本" + version)
        self.mod_vers_loc.send_keys(version)
        self.wait(2)
        logger.info("点击保存")
        self.mod_save_loc.click()
    ...
    def DelProject(self, value):
        ...
        self.line_sel_loc.select_by_value(value)
        logger.info("点击执行")
        self.execute_loc.click()
        logger.info("二次确认")
        self.line_del_loc.click()
        self.wait(3)
